...
export class RealtimeSession {
    constructor(prefix, config = {}) {
        ...
        // Kill-switch flag set by the coordinator on stop/barge-in. While true, every
        // incoming duplex SPEAK frame is dropped (audio + speak callbacks) until the
        // model returns to listen — the only reliable way to halt the released SPEAK
        // head, since the duplex protocol has no server-side abort. See _handleSpeak.
        this.duplexOutputGated = false;
        // Thinker-period re-hold: while true, a return-to-listen does NOT lift duplexOutputGated,
        // so the SPEAK head stays gated for the WHOLE Thinker turn (filler -> thinking -> answer)
        // and cannot blurt its own reflex answer over the Thinker's chat-TTS reply. Set/cleared
        // by the coordinator (holdTalkerForThinker / releaseTalker). See _handleListen.
        this.thinkerHoldActive = false;
    ...
    _handleListen(msg) {
        ...
        this._wasListening = true;
        // The model returned to listen: the (possibly stop-cut) SPEAK turn is fully over.
        // Lift the kill-switch so the model's NEXT speak turn plays normally — UNLESS a Thinker
        // turn still owns the floor (thinkerHoldActive), in which case the gate must stay up for
        // the whole turn so the duplex head can't blurt between/over the chat-TTS filler+answer.
        if (!this.thinkerHoldActive) this.duplexOutputGated = false;
    ...
    _handleSpeak(msg) {
        ...
        // Kill-switch: a stop/barge-in cut this SPEAK turn. The duplex protocol has no
        // server-side abort, so the server keeps streaming speak frames for up to ~18s.
        // Drop the rest of the turn here (audio + speak callbacks) so it can't resume —
        // this is what makes "stop" actually halt the released ear. Cleared in _handleListen
        // when the model next returns to listen.
        if (this.duplexOutputGated) {
            this._lastResultTime = recvTime;
            return;
        }
